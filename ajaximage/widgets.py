...
class AjaxFileWidget(widgets.TextInput):

    html = """
    <div class="ajaximage">
        <a class="file-link" target="_blank" href="{file_url}">
           <span class="file-lnk">{file_url}</span>
        </a>
        <a class="file-remove" href="#remove">Remove</a>
        <input class="file-path" type="hidden" value="{file_path}" id="{element_id}" name="{name}" />
        <input type="file" class="file-input" />
    ...
    def __init__(self, *args, **kwargs):
        self.upload_to = kwargs.pop('upload_to', '')
        super(AjaxFileWidget, self).__init__(*args, **kwargs)
    ...
    def render(self, name, value, attrs=None):
        ...
        upload_url = reverse('ajaxfile', kwargs=kwargs)
